...
#microsfot #windows #powershell #linux #iot


если каждый раз загружать систему из свеже созданного дочернего vhdx, то можно получить некое подобие иммьютбл системы
если после каждой загрузки использовать базовый vhdx для создания дочернего образа, а затем использовать этот образ во время следующего старата системы, то ... Система каждый раз будет выглядеть как будто она установлена только что


![alt text](article-assets/article-image.jpg)

Второй четверг каждого месяца Microsoft и некоторые другие компании выпускают регулярные обновления для своих продуктов. Это так называемый Patch Tuesday. Среди прочего, в этот день Microsoft публикует commulative update для Windows - накопительный пакет обновлений, который включает все предыдущие исправления.
